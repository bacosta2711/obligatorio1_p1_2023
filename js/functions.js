...
function initialChargeStats() {
	...
	if (avg > 0) {
		...
		text = 'El promedio de las cantidades considerando todos los reclamos de todas las empresas es: No se puede calcular el promedio. No hay reclamos';
	}
	...
	if (maxQttyCategories > 0) {
		...
		let description = 'No hay información para ninguna categoría'
		let li = document.createElement('li');
		li.textContent = description;
...
function sendClaim() {
	let form = document.getElementById('addClaimForm');
...
function filterSearch() {
	goToViewTickets_nav();
	filterClaims();
	if (system.systemClaimsFilter.length > 0) {
...
function filterClaims() {
	...
	let searchValue = document.getElementById('search_action').value.toUpperCase();
	for (let i = 0; i < system.systemClaims.length; i++) {
		let claim = system.systemClaims[i];
		let claimString = claim.claimPerson.toUpperCase() + " " + claim.claimTitle.toUpperCase() + " " + claim.claimCompany.companyName.toString().toUpperCase();

		if (claimString.includes(searchValue)) {
			system.systemClaimsFilter.push(claim);
		}
	}
}
...
function claimsGeneretor() {
	...
	for (let i = system.systemClaims.length - 1; i >= 0; i--) {
		...
		buttonOne.addEventListener('click', function () {
			span4.textContent = parseInt(span4.textContent) + 1;
			newSubs = span4.textContent;
			system.systemClaims[i].claimSubscribers = system.systemClaims[i].claimSubscribers +1;
		});
	}
}
